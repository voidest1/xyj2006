...

{
    ...
    write("         [[1;31m提示：[0;37;0m如果按"HIY"ENTER"NOR"键后系统没有响应，请试 Ctrl-ENTER]\n\n");

    write("                  幻想西天欢迎你的到来!Select GB or BIG5(gb/big5):");
...
// try strict ban here...weiqi.
	...
	// vikee 2001-2-15 17:42
          ...
          printf("幻想西天["HIG"华中站"NOR"]现有注册玩家"HIR"%s"NOR"位。\n",chinese_number(howmany_user()));
//	printf("西行战记["HIY"北京站"NOR"]开站以来，最高在线数目："HIY"%s"NOR" 人，平均在线数目："HIY"%s"NOR" 人。\n",chinese_number(max_online),chinese_number(avg_online));

	printf("目前共有"HIR"%s"NOR"位管理人员、"HIG"%s"NOR"位玩家，以及"HIY"%s"NOR"位使用者在尝试连线。\n\n",
...
*/
	...
	write("在您进入游戏前,幻想西天提醒您不要忘记您的工作或者学习\n");
...
请您给自己想一个符合〖幻想西天〗神话世界的中文名字。这

个名字将代表你在幻想西天中的人物，而且往后将不能再更改。
...

{
	...
	user->set("no_gift",1);
		...
		query_ip_number(ob), ctime(time()) ) );
    CHANNEL_D->do_channel( this_object(), "rumor","听说一颗流星坠落在长安附近，"+user->query("name")+"("+user->query("id")+")来到了人世，从此开始了他(她)的西行之路。"NOR);
...
{            
	...
	"/adm/daemons/toptend"->topten_checkplayer(user);
		...
		tell_room(startroom, user->query("name") + "连线进入这个世界。\n",
			...
			({user}));
//    message("channel:chat", HIM"〖再战江湖〗: "HIC"据说"+HIW+user->name()+HIC "来到了幻想西天，继续着他未完成的路途\n"NOR,users());
